...
use the ```KPATCH_LOAD_HOOK``` macro to specify a function to be called when
the patch module is loaded.

```kpatch-macros.h``` provides ```KPATCH_LOAD_HOOK``` and
```KPATCH_UNLOAD_HOOK``` macros to define such functions.  The signature of
both hook functions is ```void foo(void)``` and and they may run in
```stop_machine``` context (so they must not sleep).

Example: a kpatch fix for CVE-2016-5389 utilized the ```KPATCH_LOAD_HOOK```
and ```KPATCH_UNLOAD_HOOK``` macros to modify variable
```sysctl_tcp_challenge_ack_limit``` in-place:
