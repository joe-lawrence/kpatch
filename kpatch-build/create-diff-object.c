...
static bool kpatch_line_macro_change_only_aarch64(struct section *sec)
{
	...
	for (offset = 0; offset < size; offset += AARCH64_INSTR_LEN) {
		if (!memcmp(start1 + offset, start2 + offset, AARCH64_INSTR_LEN))
			continue;
					  ...
					  struct section *sec)
{
	switch(kelf->arch) {
	case AARCH64:
		return kpatch_line_macro_change_only_aarch64(sec);
	case PPC64:
